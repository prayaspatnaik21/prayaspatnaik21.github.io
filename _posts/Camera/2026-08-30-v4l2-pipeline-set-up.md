...
reading_time: 15
icon: "Camera + Jetson"
excerpt: "Learning how a camera frame reaches an application on Linux"
---
...
1. [How does the application identify and open the correct camera capture interface?](#1-opening-the-capture-device)
2. [How does it determine what that interface supports?](#2-querying-device-capabilities)
3. [How do the application and the driver agree on the frame format?](#3-configuring-the-frame-format)
4. [Where does the driver store the captured frame?](#4-requesting-and-mapping-capture-buffers)
5. [How does the application know when a frame is ready?](#6-waiting-for-a-completed-frame)
6. [How does the application access the frame and return its buffer to the driver?](#7-dequeueing-and-accessing-the-frame)

## The V4L2 capture stages

Each question corresponds to one stage of the V4L2 capture process:

1. [Open the capture device.](#1-opening-the-capture-device)
2. [Query its capabilities.](#2-querying-device-capabilities)
3. [Configure the frame format.](#3-configuring-the-frame-format)
4. [Request and map the buffers.](#4-requesting-and-mapping-capture-buffers)
5. [Queue the buffers.](#5-queueing-buffers-and-starting-the-stream)
6. [Start the video stream.](#5-queueing-buffers-and-starting-the-stream)
7. [Wait for and dequeue a completed frame.](#6-waiting-for-a-completed-frame)
8. [Process and requeue the buffer.](#8-processing-and-requeueing-the-buffer)
9. [Stop streaming and release the resources.](#9-stopping-streaming-and-releasing-resources)

### Complete capture-flow diagram
...
3. Before sending V4L2 requests, the application opens the capture node with the `open()` system call.
    ...
    ```
    - `configuration_.devicePath.c_str()` supplies the device-node path.
    - `O_RDWR` requests read and write access, which is required by many V4L2 streaming operations.
    - `O_NONBLOCK` prevents operations such as `VIDIOC_DQBUF` from waiting when no completed frame is available.
...
3. V4L2 extends standard file operations through device-specific commands called IOCTLs. The application sends an IOCTL request to the driver using the file descriptor returned by `open()`.
    ...
    - `fileDescriptor_` → identifies the open capture interface.
    - `VIDIOC_QUERYCAP` → identifies the requested operation.
    - `&capability` → points to the structure the driver will fill.
